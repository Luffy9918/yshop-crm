<template>
  <view class="customer-edit-container">
    <AppNavbar title="编辑客户" :show-back="true"></AppNavbar>
...
// 页面加载时获取客户ID
onUniLoad((options: any) => {
  if (options.id) {
    customerId.value = Number(options.id)
    loadCustomerDetail(customerId.value)
  }
})
...
async function loadCustomerDetail(id: number) {
  ...
  try {
    ...
    if (res.code === 0) {
      ...
      formData.loanAmount = data.loanAmount || ''
...
async function handleSubmit() {
  try {
    ...
    // 调用更新客户API
    const res = await updateCustomer(customerId.value, formData)
